    ...
    return {
        init: function(validationRules,table_id,arrayRule) {
            ...
            $("#m_form_submit,.m_form_submit").click(function(e) {
                ...
                if($.isArray(arrayRule)){
                    $.each(arrayRule,function (key,value) {
                        if(Object.keys(value).length > 0){
                            form.find('[name^='+value.field+']').each(function() {
                                if($(this).attr('name').indexOf('category_id') > 0){
                                    $(this).rules("add", {
                                        required: true
                                    });
                                }else{
...
$(document).ready(function () {
    ...
    var $repeater2 = $("#kt_repeater_2").repeater({
        initEmpty: !1,
        show: function() {
            $(this).slideDown()
        },
    ...
    $('.modal').on('hidden.bs.modal', function (e) {
        ...
        if($.isFunction($.fn.repeater)){
            $(e.currentTarget).find('[data-repeater-item]').not('div:first').remove();
        }
        if($.isFunction($.fn.paraia_multi_select)){
            $('.paraia-multi-select').remove();
        }
    ...
    $('.modal-data').on('shown.bs.modal', function (e) {
        ...
        var form_type = $(e.relatedTarget).data('form_type');
        if(type == 'add')
        {
        ...
        else if(type == 'edit')
        {
            ...
            var inputTag = [''];
            var repeatArray = ['category'];
            var repeatArray1 = ['product'];
            ...
            $.each(data,function (key,val) {
                ...
                {
                    setTimeout(function () {
                        $(e.currentTarget).find('.formAdd').find('#'+form_type+'_'+key).val(val).change();
                    },500);
                }else if($.isFunction($.fn.fileinput) && $.inArray(key,imgArray) != -1)

                {
                    $(e.currentTarget).find('.formAdd').find('#'+form_type+'_'+key).attr('src',val);
                }else if($.isFunction($.fn.summernote) && $.inArray(key,summerNoteArray) != -1)

                {
                    $(e.currentTarget).find('.formAdd').find('#'+form_type+'_'+key).summernote('code',val);
                }else if($.isFunction($.fn.datepicker) && $.inArray(key,dateArray) != -1)

                {
                    $(e.currentTarget).find('.formAdd').find('#'+form_type+'_'+key).datepicker('setDate', val);
                }else if($.inArray(key,radoiArray) != -1)
                {
                    $(e.currentTarget).find('.formAdd').find('#'+form_type+'_'+key+'_'+val).prop('checked',true);
                }else if($.inArray(key,repeatArray) != -1 && $.isArray(val)){
                    if(val.length > 0){
                        $repeater1.setList(val);
                    }

                }else if($.inArray(key,repeatArray1) != -1 && $.isArray(val)){

                    if(val.length > 0){
                        $repeater2.setList(val);
                    }

                }
                else{
                    $(e.currentTarget).find('.formAdd').find('#'+form_type+'_'+key).val(val);
                }
