...
class MenController extends Controller{
    ...
    {
        $page = PageBuilder::where('slug','men')->first();
        if(!empty($page)){
            if ($page['section_1']){
        ...
        return view('backend.page.men.index', ['page_title' => 'Men Page Management','page'=>$page]);
    }

    public function storeSection(){
        $data = request()->all();
        $niceNames = [
            'section_1.*.slider_text' => 'Text',
            'section_1.*.slider_text_l' => 'Text local',
        ...
        $this->validate(request(), [
            'section_1.*.slider_text' => 'required',
            'section_1.*.slider_text_l' => 'required',
        ...
        foreach ($data['section_1'] as $key=>$val){
            if(isset($val['image_url']) && !empty($val['image_url'])) {
                $file = $val['image_url'];
                ...
                $data['section_1'][$key]['image_url']= 'storage/'.$image_url;
            }else{
                if (isset($val['image_url1'])){
                    $data['section_1'][$key]['image_url']= $val['image_url1'];
                    unset($data['section_1'][$key]['image_url1']);
                }else{
    ...
    public function storeSection2(){
        ...
        PageBuilder::updateOrCreate(['slug'=>'men'],['section_2'=>json_encode($data['section_2'])]);
    }
    ...
    public function storeSection3(){
        ...
        PageBuilder::updateOrCreate(['slug'=>'men'],['section_3'=>json_encode($data['section_3'])]);
    }
}