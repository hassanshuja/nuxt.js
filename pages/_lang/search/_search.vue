	...
	<div v-if="productsList.length > 0">
        ...
        <div id="search_result">
            <div class="container-fluid">
                <div class="row">
                    <div class="search_heading" :style="'padding:25px'">
                        {{$t('search.result_for')}}  {{$route.query.query }}
                    </div>	
                </div>	
            </div>	
        </div>
		...
		<div class="container-fluid" >
			<div class="row" id="bor" style="margin:auto">
					<div class="col-sm-12 col-md-12 border_spe">
							<div class="brand_cat">
							...
							<div class="col-sm-12 col-md-3 col-lg-3">
								<div class="tab" id="mobile_hide">
                                    ...
                                    <div class="View_cloth">
										<span><b>{{$t('global.gender')}}</b></span>
	
										<div class="content_cat">
											<div class="memory">
													<button class="uncollapsible" @click.prevent="setGender(null);searchCatalogue()">{{$t('global.mix')}}</button>
													<button class="uncollapsible" @click.prevent="setGender(1);searchCatalogue()">{{$t('global.men')}}</button>
													<button  class="uncollapsible" @click.prevent="setGender(2);searchCatalogue()">{{$t('global.women')}}</button>
											</div>
										</div>
									</div>
									<div class="View_cloth">
				...
				<div class="container-fluid">
					<div class="row">
						<div class="search_heading" :style="'padding:25px'">
							...
							<span>Please check the spelling or try again with a less specific term.</span>
						</div>	
						<div class="search_sizing">
							<div class="size_show">
								<p>Trending Right Now:</p>
								<div class="form-group">S<button class="remove-btn" data-toggle="tooltip" data-placement="top" title="search"><i class="fa fa-search" aria-hidden="true"></i></button></div>
								<div class="form-group">Jackhammer.co<button class="remove-btn" data-toggle="tooltip" data-placement="top" title="search"><i class="fa fa-search" aria-hidden="true"></i></button></div>
								<div class="form-group">Vezzo<button class="remove-btn" data-toggle="tooltip" data-placement="top" title="search"><i class="fa fa-search" aria-hidden="true"></i></button></div>
								<div class="form-group">White<button class="remove-btn" data-toggle="tooltip" data-placement="top" title="search"><i class="fa fa-search" aria-hidden="true"></i></button></div>
							</div>
						</div>	
					</div>	
				</div>	
...
<script>
		...
		export default {
				...
				async asyncData ({ app,store, route }) {
						...
						let response2 = await app.$axios.$get('page/search/'+route.query.query);
				...
				methods: {
					...
					async getsubcatItem() {
						...
						let searchUrl = `${this.baseURL}/page/search/${this.query_param}
										?sizing_gender=${this.sizing_gender}&page=${this.page}&subcategory_id=${subcategory_id}`;
						if(this.colorSearch.length > 0 || this.sizeSearch.length > 0) {
							...
							searchUrl = `${this.baseURL}/page/search/${this.query_param}?sizing_gender=${this.sizing_gender}&page=${this.page}&colors=${colors}&size=${size}&subcategory_id=${subcategory_id}`;
						}
					...
					async searchCatalogue() {
						...
						console.log(sortby)
						let searchUrl = `${this.baseURL}/page/search/${this.query_param}
						?sizing_gender=${this.sizing_gender}&page=${this.page}&subcategory_id=${subcategory_id}&sortby=${sortby}`;
						if(this.colorSearch.length > 0 || this.sizeSearch.length > 0) {
							var colors =  JSON.stringify(this.colorSearch);
							var size =  JSON.stringify(this.sizeSearch);
							searchUrl = `${this.baseURL}/page/search/${this.query_param}
							?sizing_gender=${this.sizing_gender}&page=${this.page}&colors=${colors}&size=${size}
