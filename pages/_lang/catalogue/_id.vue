<template>
	<div>
		<bottom-header :categoryList="categoryList" :custom_url="$route.query.sizing_gender+'/'" :url="$route.query.sizing_gender+'/category/'"></bottom-header>
...
<script>
		...
		export default {
				...
				async asyncData ({ app,store, route }) {
						...
						//this sizing_gender will check if it is men or women 
						var sizing_gender = route.query.sizing_gender
						var tagId = route.query.tagId
						app.$axios.defaults.baseURL = process.env.baseURL
						app.$axios.setHeader('lang', store.state.locale)
						let response1 = await app.$axios.$get(sizing_gender+'/category');
						let response2 = await app.$axios.$get('page/generalcatalogue?sizing_gender='+sizing_gender+'&tag_id='+tagId);
				...
				methods: {
					...
					async searchCatalogue() {
						...
						let searchUrl = this.baseURL+'/page/generalcatalogue?sizing_gender='+this.sizing_gender+'&tag_id='+this.tagId+'&page='+this.page+'&sortby='+sortby;
						if(this.colorSearch.length > 0 || this.sizeSearch.length > 0) {
							...
							searchUrl = this.baseURL+'/page/generalcatalogue?sizing_gender='+this.sizing_gender+'&tag_id='+this.tagId+'&page='+this.page+'&colors='+colors+'&size='+size+'&sortby='+sortby;
						}
